...
validate_workspace() {
    ...
    for file in ".clinerules" ".vscode/settings.json" \
        ...
        "memory-bank/activeContext.md" "memory-bank/progress.md"; do
        if [ ! -f "$file" ]; then
...
bootstrap() {
    ...
    # Display next steps
    echo -e "\n${BLUE}Next Steps:${NC}"
    echo -e "1. Review and update memory bank documentation"
    echo -e "2. Configure development environment settings"
    echo -e "3. Begin implementing core features"
    if [ "$DOCKER_SUPPORT" = "true" ]; then
        echo -e "4. Test Docker configuration with 'docker-compose up'"
    fi
}
